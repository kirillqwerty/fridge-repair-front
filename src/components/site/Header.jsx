...
const NAV_LINKS = [
  { id: "pricing", label: "Цены", mobileLabel: "Стоимость" },
  { id: "brands", label: "Бренды" },
  { id: "portfolio", label: "Портфолио", desktop: false },
      ...
      <div className="container-page flex items-center justify-between py-3 sm:py-4 gap-3">
        ...
        >
          ...
          <div className="leading-tight">
            <div className="font-display font-semibold text-base sm:text-lg text-foreground">
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {NAV_LINKS.filter((link) => link.desktop !== false).map((link) => (
            <a
              ...
              className="px-3 py-2 text-sm text-foreground/80 hover:text-primary rounded-md hover:bg-accent/60 transition-colors"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent
              side="right"
              className="w-[88%] sm:w-[420px] bg-background"
            >
              <div className="flex flex-col gap-6 mt-6">
                ...
                <nav className="grid grid-cols-1 gap-1 mt-2">
                  {NAV_LINKS.filter((link) => link.mobile !== false).map((link) => (
                    <a
                      key={link.id}
                      href={`#${link.id}`}
                      onClick={handleSectionClick(link.id)}
